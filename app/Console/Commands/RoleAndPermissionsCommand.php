...
class RoleAndPermissionsCommand extends Command
{
    ...
    protected array $actions = [
        'Вихід' => 'finish',
        'Дозволи користувачів' => 'indexUserPermissions',
        'Список користувачів з ролями та індивідуальними дозволами' => 'indexAdmins',
        'Ролі зі списоком дозволів' => 'indexRoles',
        'Всі дозволи' => 'indexPermissions',
    ];
    ...
    protected function indexUserPermissions()
    {
        ...
        User::with('permissions')->chunk(12, function ($admins) {
    ...
    {
        $admin = User::with(['roles', 'permissions'])->findOrFail($this->ask('Введіть id користувача'));
    ...
    {
        $admin = User::with(['roles'])->findOrFail($this->ask('Введіть id користувача'));
    ...
    protected function indexAdmins()
    {
        ...
        $admins = User::with(['roles', 'permissions'])->get();
        ...
        $this->table(
            ['id', 'Імя', 'email', 'Ролі', 'Індивідуальні дозволи користувача'],
            $admins->toArray(),
            $this->styleTable,
        );
    ...
    protected function actions(array $actions)
    {
        ...
        if (array_key_exists($action, $actions) and method_exists($this, $actions[$action])) {
            $this->{$actions[$action]}();
        }
    }
